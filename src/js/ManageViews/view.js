...
class View {
	...
	init(data){
		...
		this.borderType !=="0" && this.initBorder(this.container.children(".bgSvg"));
	...
	initChart($el,data){
		...
		this.chart = new  Chart($el[0],{border,size},data);
	}

	initTable($el,data){
		const  border = this.borderType;
		const  size = this.size;
		this.table = new STable($el,{border,size},data);
	}

	initRealTime($el,data){
		const border = this.borderType;
		const size = this.size;
		this.timeReal = new TimeView($el,{border,size},data);
	}
	...
	initBorder($el){
		...
		this.border = new Border($el,{id,title,size,borderType});
	}
...
export {View,Border,STable,TimeView} ;